...
export function TagSelection({ selectedTags, onTagToggle }: TagSelectionProps) {
  ...
  return (
    <div className="w-full mb-8 mt-20">
      <h3 className="text-2xl font-athiti text-gray-800 mb-6 text-center">Tags</h3>
        ...
        {TAG_GROUPS.map((group, groupIndex) => (
          <div key={group.title} className="flex items-center gap-4">
            {group.tags.map((tag, tagIndex) => {
              ...
              return (
                <div key={tag} className="flex flex-col items-center min-w-[60px]">
                  <button
                    type="button"
                    onClick={() => onTagToggle(tag)}
                    className={`w-12 h-12 p-0 border-2 rounded-lg bg-white transition-all duration-200 ${
                      isSelected 
                  ...
                  >
                    <img 
                      src={`https://res.cloudinary.com/meal-creator/image/upload/v1662276052/icons/${iconName}.png`}
                      alt={tag}
                      className={`w-8 h-8 mx-auto ${!isSelected ? 'grayscale opacity-60' : ''}`}
                    />
                  </button>
                  <span className="text-xs font-athiti text-gray-700 mt-1 capitalize text-center">
                    {tag}
                  </span>
                </div>
              );
