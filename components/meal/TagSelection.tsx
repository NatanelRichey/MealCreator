...
export function TagSelection({ selectedTags, onTagToggle }: TagSelectionProps) {
  ...
  return (
    <div className="w-full mb-1 sm:mb-2 lg:mb-8 mt-2 sm:mt-4 lg:mt-20">
      <h3 className="text-base sm:text-xl lg:text-2xl font-athiti text-gray-800 mb-1 sm:mb-2 lg:mb-6 text-center">Tags</h3>
      
      <div className="flex flex-wrap justify-center items-center gap-1 sm:gap-3 md:gap-4 lg:gap-6 w-full">
        {TAG_GROUPS.map((group, groupIndex) => (
          <div key={group.title} className="flex items-center gap-1 sm:gap-3 md:gap-4">
            {group.tags.map((tag, tagIndex) => {
              const isSelected = selectedTags.includes(tag);
              const iconName = getIconName(tag);
              
              return (
                <div key={tag} className="flex flex-col items-center min-w-[40px] sm:min-w-[60px]">
                  <button
                    type="button"
                    onClick={() => onTagToggle(tag)}
                    className={`w-8 h-8 sm:w-12 sm:h-12 p-0 border-2 rounded-md sm:rounded-lg bg-white transition-all duration-200 ${
                      isSelected 
                  ...
                  >
                    <CldImage 
                      ...
                      quality="auto"
                      format="auto"
                      className={`w-5 h-5 sm:w-8 sm:h-8 mx-auto ${!isSelected ? 'grayscale opacity-60' : ''}`}
                    />
                  </button>
