...
export function Pantry({ items = [] }: PantryProps) {
  ...
  // Separate in-stock and saved items
  const inStockItems = pantryItems.filter(item => item.inStock);
  const savedItems = pantryItems.filter(item => !item.inStock);
  ...
  const handleAddItem = async (category: string, itemName: string) => {
    ...
    try {
      ...
      const realItem = await addPantryItem(category, itemName.trim());
      
      // Replace temp item with real item
      setPantryItems(prev => 
        prev.map(item => item._id === tempId ? { ...realItem, inStock: true, quantity: 1, unit: 'piece' as const, addedDate: realItem.createdAt } : item)
      );
      ...
      setPantryItems(prev => prev.filter(item => item._id !== tempId));
    }
  };
  ...
  const handleToggleItem = async (itemId: string) => {
    ...
    try {
      if (item.inStock) {
        // Moving to saved items
        await moveToSavedItems(item.name);
      } else {
        // Moving back to category
        await moveFromSavedItems(item.name);
      }
  ...
  const handleEditItem = async (itemId: string, newName: string) => {
    ...
    try {
      await editPantryItem(itemId, newName.trim());
